...
try {
    ...
    if (isset($_POST['txtrt_description'])) {
        $description = trim(stripslashes($_POST['txtrt_description']));
    }
    if (isset($_POST['cbostatus'])) {
        $status = trim(stripslashes($_POST['cbostatus']));
    }

    $sql_select = "SELECT * FROM hrms_rate_types WHERE rt_description = ?";
    ...
    $sql_insert = "INSERT INTO hrms_rate_types(rt_description, rt_status) VALUES (?,?)";
    ...
    header('Location: ../Views/RateType/List.php');
} catch (Exception $e) {
